...
for (x in str_vec) {
  ...
  x1 <- sqrt(num)
  x2 <- (num) * 2
  calculations_vec <- c(calculations_vec, x1, x2, x1 + x2)
}
calculations_vec
...
for (num in double_vec) {
  vec_sum <- vec_sum + num
  vec_count <- vec_count + 1
}

vec_avg <- vec_sum / vec_count
vec_mean <- mean(double_vec)
vec_sd <- sd(double_vec)
sprintf("sum: %f", vec_sum)
sprintf("avg: %f", vec_avg)
sprintf("mean: %f", vec_mean)
sprintf("standard derivation: %f", vec_sd)
```
...
consumption_sum <- sum(df_consumptions)
consumption_count <- nrow(df)
result <- consumption_sum / consumption_count
result

result_mean <- mean(df_consumptions)
result_mean

result_sd <- sd(df_consumptions)
result_sd
