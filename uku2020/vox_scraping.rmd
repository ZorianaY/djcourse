...
economicnews <- read_html(link)    # nr: це перший, пробний url
...
for(page in 1:npages) {
  ...
  current_titles <- economicnews %>%    # nr: тут ви сформували працильний лінк, але берете дані зі старого html на 14 рядку. Забули прочитати html за лінком 'current_link'
    html_nodes('.entry-title') %>%
    html_text()
...
```{r}
# nr: перевірка, чи рядки унікальні
